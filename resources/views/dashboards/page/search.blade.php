...
<div class="mb-8 inline-block relative">
    ...
    <form action="{{route('dashboard.page.search')}}" method="GET" class="flex">
...
<div class="w-full">
    <div class="flex justify-between items-center mb-6">
        <div>
            <span>{{ ($pages->currentPage() -1) * $pages->perPage() + 1}}件〜{{ (($pages->currentPage() -1) * $pages->perPage() + 1) + (count($pages) -1)  }}件を表示</span>
            <span>(全{{ $pages->total() }}件)</span>
        </div>
        <div class="flex items-center">
            <span class="mr-3">並び替え</span>
            <form action="{{route('dashboard.page.index')}}" method="GET" class="flex" id="sortForm">
    ...
    <div class="flex-none mb-4 px-0 flex items-center">
        <form action="{{route('dashboard.page.bulkDestroy')}}" class="mr-8 js-delete-form hidden" method="POST">
            @csrf
            @method('DELETE')
    ...
    <div class="border border-gray-200 bg-white">
        <a href="{{route('dashboard.stamp.edit', ['stamp'=>$page->id]) }}" class="relative flex items-center px-4 py-2 text-sm shadow-sm font-medium hover:bg-gray-100 p-4 ">
            ...
            <div class="flex justify-center mr-1 md:mr-8 flex-none w-20">
                <img class="rounded-t-lg" src="{{Storage::url($page->image_url)}}" width="120" alt="" />
            </div>
            <div class="md:flex md:grow items-center p-2 md:p-5">
                <div class="md:flex md:grow">
                    <h5 class="flex-none w-[120px] mb-2 mr-20 text-xl font-bold text-left tracking-tight text-gray-900 ">{{$page->title}}</h5>
                    <p class="text-xs md:text-sm h-[3rem] md:h-[5rem] overflow-hidden overflow-ellipsis">{{$page->description}}</p>
                </div>
                <form action="{{route('dashboard.stamp.destroy',['stamp' => $page->id])}}" method="POST" class="mt-4">
                    @csrf
                    @method('DELETE')
